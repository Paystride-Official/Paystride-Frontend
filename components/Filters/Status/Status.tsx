...
const Status = ({ filters, setFilters }: Props) => {
    ...
    useEffect(() => {
        const updatedFilters = { ...filters, status: selectedOption };
        setFilters(updatedFilters); // Update parent filter object
    }, [selectedOption]);

    const options = [
        { value: "active", option: "Active only" },
        { value: "inactive", option: "Inactive only" },
    ];
    ...
    return (
        <div>
            <div className="capitalize">
                ...
                <p className="text-[#333333B2] mb-1">status</p>
            </div>
